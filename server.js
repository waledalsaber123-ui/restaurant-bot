...
app.post("/webhook", async (req, res) => {
  ...
  if (phone && session.pendingOrder) {
    session.pendingOrder.phone = phone;
  }

  /* ================= التحقق من التأكيد ================= */

  const isConfirmation = /^(تم|ok|okay|تمام|اوكي|yes)$/i.test(userMessage);
...
الطلب:
${order.items || "غير محدد"}

المجموع: ${order.total || "0"} دينار
`;

    await sendWA(SETTINGS.KITCHEN_GROUP, kitchenMessage);

    await sendWA(chatId,
`تم تأكيد طلبك يا غالي ✅

طلبك وصل للمطبخ
التجهيز خلال 30 - 45 دقيقة

شكراً لاختيارك مطعم صابر جو سناك ❤️`
);
  ...
  try {
    ...
    if (reply.includes("[KITCHEN_GO]")) {
      ...
      const kitchenPart = parts[1];

      session.pendingOrder = {
        name: kitchenPart.match(/الاسم:? (.*)/)?.[1],
        phone: kitchenPart.match(/الرقم:? (.*)/)?.[1],
        address: kitchenPart.match(/العنوان:? (.*)/)?.[1],
        items: kitchenPart.match(/الطلب:([\s\S]*?)المجموع/)?.[1],
        total: kitchenPart.match(/المجموع:? ([0-9.]+)/)?.[1]
      };
...
اكتب "تم" للتأكيد ✅`
    ...
    await sendWA(chatId,
"صار ضغط رسائل عندي يا غالي، جرب بعد لحظة 🙏"
);
...
app.listen(3000, () => {
  console.log("🤖 صابر جو سناك يعمل الآن");
});
